...
export default function cmsPicker(callback, value, meta) {
    const vm = this;
    ...
    (function onInit() {
        ...
        vm.dialogInstanceApi = tinymce.activeEditor.windowManager.open(vm.dialogConfig);
        changePage();
    }())
    ...
    function setPaginationButtonsDisabled(): void {
        const $prevPage = document.querySelector('.tox-button[title="previous page"]') as HTMLInputElement
        const $nextPage = document.querySelector('.tox-button[title="next page"]') as HTMLInputElement
    ...
    function isLastPage(): boolean {
        const lastPage = Math.floor(vm.list.Items.length / vm.list.Meta.PageSize) + 1;
        return vm.list.Meta.Page === lastPage;
    }

    function selectAsset(event) {
        vm.dialogInstanceApi.close();
        const imageUrl = event.currentTarget.children[0].src;
